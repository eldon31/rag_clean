...
"""
Kaggle GPU Script: Embed qdrant_ecosystem collection (Self-Contained)

USAGE IN KAGGLE:
    1. Upload this script to Kaggle notebook
    2. Add qdrant-ecosystem-chunked as input dataset
    3. Set accelerator to GPU T4 x2
    4. Run: !python embed_qdrant_ecosystem_self_contained.py
    5. Download output from /kaggle/working/

REQUIREMENTS:
    - GPU: Tesla T4 x2 (15.83 GB VRAM each)
    - Input dataset: qdrant_ecosystem_chunked/
...
class UniversalEmbedder:
    ...
    def __init__(self, config: EmbedderConfig):
        ...
        # Load model(s)
        if self.use_data_parallel:
            logger.info(f"'''n🚀 Loading models with DATA PARALLELISM (2 GPUs)")
            ...
            logger.info(f"'''n🚀 Loading single model")
        ...
        logger.info(f"✓ Verified dimension: {actual_dim}'''n")
    ...
    def embed_all(self, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"✓ Embedded {len(embeddings_data)} chunks'''n")
    ...
    def save_jsonl(self, embeddings: List[Dict[str, Any]]):
        ...
        logger.info(f"✓ Saved {lines_written} embeddings'''n")
